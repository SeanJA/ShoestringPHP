 ...
 */
	...
	function __construct($useragent = '') {
		if(!$useragent){
			$this->userAgent = 'php-twitter/1.x - To report abuse, contact ' . $_SERVER['SERVER_ADMIN'];
		} else {
	...
	function update( $status, $replying_to = false ) {
		...
		return $this->formatReturn( $this->process($request, $postargs) );
	}
	 ...
	 * @param integer $sinceId Optional. Returns results posted that have an ID greater than $since_id
	 * @return string
	 **/
	function getReplies( $page = false, $since = false, $sinceId = false ) {
		...
		return $this->formatReturn( $this->process( $request ) );
	}
	...
	function deleteStatus( $id ) {
		...
		return $this->formatReturn( $this->process( $request, true ) );
	}
	 ...
	 * @param boolean|integer $user_id specifies the id of a twitter user
	 * @param boolean|string $screen_name specifies the twitter name
	 * @param boolean|integer $since_id Narrows the returned results to just those statuses created after the specified status id.
	 * @param boolean|integer $max_id Narrows the returned results to just those status up to the status id specified
	 * @param boolean $page specifies whetther to page results
	...
	function updateAvatar( $file ) {
		...
		return $this->formatReturn( $this->process( $request, $postdata ) );
	}
	...
	function twitterAvailable() {
		...
		if( $this->formatReturn( $this->process($request) ) == '"ok"' )
			return true;
	 ...
	 * @param string $data
	 */
	private function formatReturn( $data ) {
		switch($this->type){
			case 'json':
...
class summize extends sTwitter {
/**
